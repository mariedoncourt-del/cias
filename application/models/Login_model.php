...
class Login_model extends CI_Model
{
    ...
    /**
     * Compte les tentatives échouées depuis une IP dans les 15 dernières minutes
     */
    function countFailedAttempts($ip, $minutes = 15)
    {
        $since = date('Y-m-d H:i:s', strtotime("-{$minutes} minutes"));
        $this->db->where('ip', $ip);
        $this->db->where('success', 0);
        $this->db->where('createdDtm >=', $since);
        return $this->db->count_all_results('tbl_login_attempts');
    }

    /**
     * Vérifie si une IP est bloquée (>= 5 échecs en 15 min)
     */
    function isIpBlocked($ip)
    {
        return $this->countFailedAttempts($ip, 15) >= 5;
    }
